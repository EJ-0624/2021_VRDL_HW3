...
# pip install plotly
import os
root = "C:/Users/User/OneDrive/桌面/NYCU/hw3/dataset"
os.chdir(root)
...
#     path = test[index]
#     filename = './data/test_jpg/' + path.replace('.png', '.jpg')   
#     imgs = cv2.imread('./test/' + path)
...
for index in range(len(data_path)):
    ...
    img = PIL.Image.open('./train/' + path + '/images/' + path + '.png').convert('RGB')
    # plt.imshow(img)
    # plt.axis('off')
    # plt.show()
    ...
    for i in mask_path:
        if i == ".ipynb_checkpoints":
            continue
        masks = PIL.Image.open('./train/' + path + '/masks/' + i)
